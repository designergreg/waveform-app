...
function applyActionbarVisibility() {
  ...
  // v3: "visible" = expanded, "not visible" = collapsed
  if (actionbarVisible) {
  ...
  // Always clickable / visible; we only change layout via the class
...
function draw(ts = performance.now()) {
  ...
  buttons.forEach((btn) => {
    if (window.isPTTOn) {
      ...
      // PTT OFF → only show buttons when the bar is expanded
      btn.style.display = actionbarVisible ? "flex" : "none";
    }
  });
...
if (touchTarget) {
  ...
  // Click on the *collapsed* actionbar to expand it again in PTT OFF mode
  if (actionbar) {
    actionbar.addEventListener("click", (e) => {
      // Only care in PTT OFF
      if (window.isPTTOn) return;

      // If it's already expanded, let the normal button handlers work
      if (actionbarVisible) return;

      // Don't fight with active buttons/menus (shouldn't matter when collapsed,
      // but keeps symmetry with touchTarget behavior)
      if (isAnyActionbarButtonActive()) return;

      // Expand and restart auto-hide
      actionbarVisible = true;
      applyActionbarVisibility();
      scheduleAutoHide();
    });
